...
void transpose_block_16(uint64_t *src, uint64_t *dst, uint32_t src_skip,
			uint32_t dst_skip)
{
	transs_block_16_t tmp;
	AS_UINT64(&tmp.line0) = *src;
	src += src_skip;
	AS_UINT64(&tmp.line1) = *src;
	src += src_skip;
	AS_UINT64(&tmp.line2) = *src;
	src += src_skip;
	...
	*dst = AS_UINT64(&tmp.line0);
	dst += dst_skip;
	*dst = AS_UINT64(&tmp.line1);
	dst += dst_skip;
	*dst = AS_UINT64(&tmp.line2);
	dst += dst_skip;
